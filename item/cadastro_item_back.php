...
$sql=$conecta->query("INSERT INTO item (nomeitem, documento_compra, codigo, descricao, valoraquisicao, valorvenda, quantidade_estoque, quantidade_vendida, saldo)
 VALUES ('{$nomeitem}', '{$docfiscal}', '{$codigo}', '{$descricao}', '{$valoraquisicao}','{$valorvenda}', '{$qtde_estoque}', '{$qtde_vendida}', '{$saldoitem}')");

if ($sql==true)
{
    echo '<script language="javascript">';
    echo "alert('Item cadastrado com sucesso!')";
    echo '</script>';	
...
{
    echo '<script language="javascript">';
    echo "alert('Erro na gravação do item!')";
    echo '</script>';	
}
